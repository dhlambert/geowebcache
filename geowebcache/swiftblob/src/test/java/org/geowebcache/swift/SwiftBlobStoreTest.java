...
public class SwiftBlobStoreTest {
    ...
    public void put() {
        ...
        // Create test metadata object - depends if this can be used in verifying the objectApi put
        // method
        Map<String, String> testObjectMetadata = new HashMap<>();
        testObjectMetadata.put("contentLength", contentLen);
        testObjectMetadata.put("contentType", mimeType);
        ...
        when(testResource.getSize()).thenReturn(Long.parseLong(contentLen));
        ...
        try {
            ...
            verify(keyBuilder, times(2)).forTile(testTileObject);

            // Test if listeners are empty
            verify(objectApi, times(0)).get(testKey);

            // Verify put call and arguments
            verify(objectApi, times(1))
                    .put(
                            testKey,
                            new InputStreamPayload(testInputStream),
                            PutOptions.Builder.metadata(testObjectMetadata));
            ...
            verify(this.keyBuilder, times(3)).forTile(testTileObject);
